    ...
    optimize=tf.train.AdamOptimizer(0.01).minimize(cost)
    ...
    with tf.Session() as sess:
        ...
        for i in range(iter_batch_num):
            ...
            COST,RATE,pred,OPTMIZE=sess.run([cost,correct_rate,py_x,optimize],feed_dict={X: batch[0], Y: batch[1], p_keep_conv: 0.75,p_keep_hidden: 0.75})
            print(pred[0])
            print(batch[1][0])
            print(RATE)
